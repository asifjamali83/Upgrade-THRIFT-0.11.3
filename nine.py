GNU nano 3.2                             inne.py                                        
  ...
  GNU nano 3.2                             inne.py                              Modified  
        if op.type == 11:
                if not op.param2 in admin:
                    try:                                                                                          gs = ki.getGroup(op.param1)
                        gs = kk.getGroup(op.param1)
                        targets = [op.param2]
                        for target in targets:
                           try:
                                wait["blacklist"][target] = True                                                          f=codecs.open('st2__b.json','w','utf-8')
                                json.dump(wait["blacklist"], f, sort_keys=True, indent=4,$                           except:
                            return
                    except Exception as e:
                        print (e)                                                                 #------Kick Auto Bl Finish---------#
        #-------Kick Bl Join Start---------#
  GNU nano 3.2                             inne.py                                                                    random.choice(KAC).kickoutFromGroup(op.param1,[op.param2])
                        except:                                                                                       print ("client Kick regulation or Because it does not exist i$                        if op.param2 in wait["blacklist"]:
  GNU nano 3.2                             inne.py                                        # -*- coding: utf-8 -*-
import LINETCR
from LINETCR.lib.curve.ttypes import *
from datetime import datetime
import time,random,sys,json,codecs,threading,glob,re
#kk = LINETCR.LINE()                                                                      #kk.login(qr=True)                                                                        #kk.loginResult()
#cl = LINETCR.LINE()
#cl.login(qr=True)
#cl.loginResult()                                                                         
#ki = LINETCR.LINE()                                                                      #ki.login(qr=True)
#ki.loginResult()                                                                         #kk = LINETCR.LINE()
#kk.login(qr=True)
#kk.loginResult()                                                                         
#kc = LINETCR.LINE()
#kc.login(qr=True)
#kc.loginResult()                                                                           GNU nano 3.2                             inne.py                              Modified  root@vultr:~/dika15# autopep8                                                             Command 'autopep8' not found, but can be installed with:                                  apt install python-autopep8                                                               root@vultr:~/dika15# apt install python-autopep8                                          Reading package lists... Done                                                             root@vultr:~/dika15# autopep8
usage: autopep8 [-h] [--version] [-v] [-d] [-i] [--global-config filename]                                [--ignore-local-config] [-r] [-j n] [-p n] [-a]                                           [--experimental] [--exclude globs] [--list-fixes]                                         [--ignore errors] [--select errors] [--max-line-length n]
                [--line-range line line] [--hang-closing] [--exit-code]                                   [files [files ...]]
autopep8: error: incorrect number of arguments
root@vultr:~/dika15# autopep8 inne.py                                                     # -*- coding: utf-8 -*-                                                                   import LINETCR                                                                            from LINETCR.lib.curve.ttypes import *                                                    from datetime import datetime
import time                                                                               import random
import sys                                                                                import json                                                                               import codecs                                                                             import threading
import glob                                                                               import re
#kk = LINETCR.LINE()
# kk.login(qr=True)                                                                       # kk.loginResult()
...
Bots = [mid, Amid, Bmid, Cmid, Dmid, Emid, Fmid,
        Gmid, Hmid, Imid, Jmid, Kmid, Lmid, Mmid, Nmid]
    ...
    try:
        ...
        #------Protect Group Kick finish-----#
                    ...
                    if msg.from_ in admin:
                        wait["blacklist"] = {}
                        random.choice(KAC).sendText(
                            msg.to, "Clear All Ban Done")
        ...
        if op.type == 13:
            if wait["Protectcancl"] == True:
                if op.param2 not in Bots:
                    ...
                    random.choice(DEF).cancelGroupInvitation(
                        op.param1, gMembMids)
                ...
                try:
                    ...
                    for target in targets:
                        try:
                            ...
                            json.dump(
                                wait["blacklist"], f, sort_keys=True, indent=4, ensure_ascii=False)
                        except:
                            return
        ...
        if op.type == 17:
            if op.param2 not in Bots:
                ...
                matched_list = []
                for tag in wait["blacklist"]:
                    matched_list += filter(lambda str: str ==
                                           tag, joinblacklistX)
                if matched_list == []:
                    pass
                else:
                ...
                if mid in op.param3:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print (
                                "client Kick regulation or Because it does not exist in the group、\n["+op.param1+"]\nの\n["+op.param2+"]\nを蹴る事ができませんでした。\nブラックリ ストに追加します。")
                ...
                if Dmid in op.param3:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print (
                                "client Because it is not in the group or Because it does not exist in the group \n["+op.param1+"]\nOf\n["+op.param2+"]\n I could not kick \n Add it to the black list.")
                        if op.param2 in wait["blacklist"]:
                            pass
                        if op.param2 in wait["whitelist"]:
                            pass
        ...
        if op.type == 26:
            ...
            if msg.contentType == 16:
                url = msg.contentMetadata(
                    "line://home/post?userMid="+mid+"&postId="+"new_post")
        ...
        #----------------Fungsi Allbio Finish-----------------#
                ...
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        cl.sendText(
                            msg.to, "Can not be used outside the group")
                    else:
            ...
            # elif "gurl" == msg.text:
                ...
                cl.sendText(msg.to, "Auto Join Group by Ticket is %s" %
                            str(wait["atjointicket"]))
                ...
                for ticket_id in n_links:
                    if wait["atjointicket"] == True:
                        ...
                        cl.sendText(msg.to, "Sukses join ke grup %s" %
                                    str(group.name))
            elif msg.text == "Ginfo":
                if msg.toType == 2:
            ...
            #-------------Fungsi BC Finish-------------------#
                ...
                try:
                    ...
                    if strnum == "off":
                        ...
                        if wait["lang"] == "JP":
                            cl.sendText(
                                msg.to, "Invitation refused turned off\nTo turn on please specify the number of people and send")
                        else:
                            cl.sendText(
                                msg.to, "å…³äº†é‚€è¯·æ‹’ç»�ã€‚è¦�æ—¶å¼€è¯·æŒ‡å®šäººæ•°å�‘é€�")
                    else:
                        num = int(strnum)
                        wait["autoCancel"]["on"] = True
                        if wait["lang"] == "JP":
                            cl.sendText(
                                msg.to, strnum + "The group of people and below decided to automatically refuse invitation")
                        else:
                            cl.sendText(
                                msg.to, strnum + "ä½¿äººä»¥ä¸‹çš„å°�ç»„ç”¨è‡ªåŠ¨é‚€è¯·æ‹’ç»�")
                except:
                    if wait["lang"] == "JP":
                ...
                if wait["lang"] == "JP":
                    ...
                    cl.sendText(
                        msg.to, "The automatic appending information is set as followsã€‚\n\n" + wait["message"])
        ...
        #-------------Fungsi Add Admin Start------------------#
            ...
            #-------------Fungsi Jam Update Finish-------------------#
                ...
                if msg.to in wait2['readPoint']:
                    ...
                    cl.sendText(
                        msg.to, "An already read point has not been set.\n「set」you can send ♪ read point will be created ♪")
# -----------------------------------------------
...
# -----------------------------------------------
    ...
    #-------------Fungsi Tag All Start---------------#
                ...
                msg.text = cb2
                msg.contentMetadata = {
                    'MENTION': '{"MENTIONEES":['+cb+']}', 'EMTVER': '4'}
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                if target not in Bots:
                                    try:
                                        klist = [kj, kd, kn, km, kl, ki,
                                                 kk, kc, ks, ka, kb, ko, ke, ku]
                                        kicker = random.choice(klist)
                            ...
                            for target in targets:
                                try:
                                    ...
                                    json.dump(
                                        wait["blacklist"], f, sort_keys=True, indent=4, ensure_ascii=False)
                                    cl.sendText(
                                        msg.to, "Akun telah sukses di banned")
                                except:
                                    ki.sendText(msg.to, "Error")
...
#-------------------Fungsi Spam Start----------------------#
                ...
                if msg.from_ in admin:
                    txt = msg.text.split(" ")
                    jmlh = int(txt[2])
                    teks = msg.text.replace(
                        "Spam "+str(txt[1])+" "+str(jmlh) + " ", "")
                    tulisan = jmlh * (teks+"\n")
                # Keke cantik <3
                    if txt[1] == "on":
                        if jmlh <= 1000:
  ...
  #----------------------Fungsi Spam Finish-------------------------------#
      ...
      #-------------Fungsi Bannlist Finish------------------#
                    ...
                    if msg.toType == 2:
                        ...
                        try:
                            ...
                            for var in range(0, num):
                                name = "".join([random.choice(source_str)
                                                for x in xrange(10)])
                                time.sleep(0.01)
                                group.name = name
                ...
                try:
                    source_str = 'abcdefghijklmnopqrstuvwxyz1234567890@:;./_][!&%$#)(=~^|'                    name = "".join([random.choice(source_str)
                                    for x in xrange(10)])
                    anu = msg.text.replace("fakecat'", "")
                    cl.sendText(msg.to, str(
                        cl.channel.createAlbum(msg.to, name, anu)))
                except Exception as e:
                    try:
            ...
            try:
                if op.param1 in wait2['readPoint']:
                    ...
                    if Nama in wait2['readMember'][op.param1]:
                        ...
                        wait2['setTime'][msg.to] = datetime.today().strftime(
                            '%Y-%m-%d %H:%M:%S')
                else:
                    cl.sendText
            except:
...
def nameUpdate():
  ...
  GNU nano 3.2                             inne.py                                        
                            ...
                            print ("clientが蹴り規制orグループに存在しない為、\n["+op.par$                        if op.param2 in wait["blacklist"]:
                            pass
  GNU nano 3.2                             inne.py                                        
                    else:
                        wait["blacklist"][op.param2] = True
  ...
  GNU nano 3.2                             inne.py                                        
                        ...
                        ki.sendText(msg.to,"deleted")
  GNU nano 3.2                             inne.py                                        
                        kk.sendText(msg.to,"Can not be used outside the group")                               else:
                        kk.sendText(msg.to,"Not for use less than group")
            elif msg.text in ["Cv3 curl","Cv3 link off"]:
                if msg.toType == 2:                                                                           X = kc.getGroup(msg.to)
  GNU nano 3.2                             inne.py                                                                        contact2 = cl.getContact(target).displayName
                                copy = cl.getProfile()
                                copy.statusMessage = contact                             $                                copy.displayName = contact2
  GNU nano 3.2                             inne.py                                                                        cl.acceptGroupInvitationByTicket(group.mid,ticket_id)
                                cl.sendText(msg.to,"Sukses join ke grup %s" % str(group.n$
            elif msg.text == "Ginfo":                                                                     if msg.toType == 2:
  GNU nano 3.2                             inne.py                                                        ko.sendText(msg.to,Gmid)                                                                  ke.sendText(msg.to,Hmid)
                ku.sendText(msg.to,Imid)                                                              elif "RA 1" == msg.text:                                                        GNU nano 3.2                             inne.py                                                            else:
                        cl.sendText(msg.to,"done")
                else:                                                                                         wait["Protectjoin"] = True                                                                if wait["lang"] == "JP":                                                GNU nano 3.2                             inne.py                                          GNU nano 3.2                             inne.py                              Modified                          return                                                                                for jj in matched_list:
                        try:                                                                                          klist=[ki,kk,kc]                                                                          kicker=random.choice(klist)
                            kicker.kickoutFromGroup(msg.to,[jj])
                            print (msg.to,[jj])
                        except:
                            pass                                                                   #----------------Fungsi Banned Kick Target Finish----------------------#        $
            elif "~" in msg.text:                                                                       if msg.from_ in Bots:                                                                       if msg.toType == 2:                                                                           print ("ok")                                                                              _name = msg.text.replace("~","")                                                          gs = ki.getGroup(msg.to)
                    gs = kk.getGroup(msg.to)                                                                  gs = kc.getGroup(msg.to)
                    ki.sendText(msg.to,"")                                                                    kk.sendText(msg.to,"")                                                                    kc.sendText(msg.to,"")                                                                    msg.contentType = 13
                    msg.contentMetadata = {'mid': mid}
                    ks.sendMessage(msg)
                    targets = []
                    for g in gs.members:                                                    GNU nano 3.2                             inne.py                                                    elif "Nk " in msg.text:
                  if msg.from_ in admin:
                       nk0 = msg.text.replace("Nk ","")
                       nk1 = nk0.lstrip()                                                                        nk2 = nk1.replace("@","")                                            GNU nano 3.2                             inne.py                                        
                        kc.sendText(msg.to,"Tidak Ditemukan.....")
                    else:                                                                                         for target in targets:                                              GNU nano 3.2                             inne.py                                                                    targets.append(g.mid)                                                             if targets == []:
                        cl.sendText(msg.to,"Tidak Ditemukan.....")
                        ki.sendText(msg.to,"Tidak Ditemukan.....")                          GNU nano 3.2                             inne.py                              Modified                     tulisan = jmlh * (teks+"\n")                                                             #Keke cantik <3
                   if txt[1] == "on":
                        if jmlh <= 1000:                                                                               for x in range(jmlh):
                                   cl.sendText(msg.to, teks)                                GNU nano 3.2                             inne.py                                        
                            name = "".join([random.choice(source_str) for x in xrange(10)$
                            time.sleep(0.01)                                                                          group.name = name                                                                         cl.updateGroup(group)                                                             except:
  GNU nano 3.2                             inne.py                                          GNU nano 3.2                             inne.py                                        
# -*- coding: utf-8 -*-                                                                   import LINETCR                                                                            from LINETCR.lib.curve.ttypes import *                                                    from datetime import datetime
import time,random,sys,json,codecs,threading,glob,re

#kk = LINETCR.LINE()                                                                      #kk.login(qr=True)                                                                        #kk.loginResult()                                                                         #cl = LINETCR.LINE()
#cl.login(qr=True)                                                                        #cl.loginResult()                                                                         
#ki = LINETCR.LINE()                                                                      #ki.login(qr=True)                                                                        #ki.loginResult()                                                                         
#kk = LINETCR.LINE()                                                                      #kk.login(qr=True)
#kk.loginResult()                                                                         #kc = LINETCR.LINE()
#kc.login(qr=True)
#kc.loginResult()

#ks = LINETCR.LINE()                                                                      #ks.login(qr=True)                                                                        #ks.loginResult()

cl = LINETCR.LINE()
cl.login(token="EF8Z9oa2qBhZHP2f3OC0.nvD/uRjfRoBETK4ZOWhZSa.AtqUkO/fJZCLQtN6VBWxTjQ3Xv8Kd$
kj = LINETCR.LINE()                                                                       kj.login(token="EF08RMHcvNLk8OS248P2.XhCwJBakhq3WIw4uZqu08G.fsJoC1T2F5hhSFOfUJ/00yKXhy7Uj$
kd = LINETCR.LINE()
kd.login(token="EF40lMHhHf6b3nU54FV1.f04Z5tIwN8oo0wvkaqNsqq.k1ATLrBmQEhLOKu3kBJS5vOrbYNZD$
kn = LINETCR.LINE()                                                                       kn.login(token="EFBJR4qnFOPMAXUQC2Fd.JZJ/1Jhb9oyNKqIA+uiyFq.Rr4ydC+G6+bikVCL+4iZWhukvTdDE$
km = LINETCR.LINE()
km.login(token="EFImLyxaCW6n1o8zCEn7.t/IdFcgcW7pVEtQdsL6WzW.BC7t8pWN1zwENxAyYA+WPvUL5T2H+$
kl = LINETCR.LINE()
kl.login(token="EFG6zgmRWrsHYhsXRYz9.sq7xakrN1v28/yW8CSq42q.BnZVyBT7+4ztkJz895WuUaGaO3Rlb$
ki = LINETCR.LINE()
ki.login(token="EF9sJWLCoUfoxPdKuzy9.4vNGKfbDYS0efY/RcqAJMq.iVBnyBMpRSISG0sJiGkjg2feGmR4U$kk = LINETCR.LINE()
kk.login(token="EFuzRkr6Lovev2DZuLib.bAXEGo6vXQI0VRhomlb3MW.QC2vU8iyQ6lt+1Q3YFkEVn11PRPd6$
kc = LINETCR.LINE()
kc.login(token="EFLxw9sumRWGOKOzWx87.4ciXpOp+ngV1ye23dcM5vW./Wcqb1J9BJEcXrjEEbtR4x+vBOsqp$
